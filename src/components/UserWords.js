...
const InitWords = () => {
    ...
    useEffect(() => {
        const fetchUserWords = async () => {
            try {
                const response = await axios.get("https://z6o1ltqubi.execute-api.eu-north-1.amazonaws.com/default/api/getuserwords");
                setUserWords(response.data.body);
            } catch (error) {
    ...
    useEffect(() => {
        if (UserWords.length > 0) {
            setCurrentWord(UserWords[currentIndex].word);
        }
    }, [UserWords, currentIndex]);
    ...
    const handleNextWord = () => {
        setCurrentIndex((prevIndex) => (prevIndex < UserWords.length - 1 ? prevIndex + 1 : prevIndex));
    ...
    return (
        <div className="container">
            <h2 className="text-center mt-5">YOUR WORDS</h2>
            {!(currentIndex === UserWords.length - 1 && atLastWord) && (
                <div>
                    ...
                    <div className="progress-bars">
                        <div className="progress">
                            <div
                                ...
                                role="progressbar"
                                style={{ width: `${(score.greens / UserWords.length) * 100}%` }}
                                aria-valuenow={(score.greens / UserWords.length) * 100}
                                aria-valuemin="0"
                                aria-valuemax="100"
                            />
                        </div>
                        <div className="progress mt-2">
                            <div
                                ...
                                role="progressbar"
                                style={{ width: `${(score.reds / UserWords.length) * 100}%` }}
                                aria-valuenow={(score.reds / UserWords.length) * 100}
                                aria-valuemin="0"
                                aria-valuemax="100"
                            />
                        </div>
